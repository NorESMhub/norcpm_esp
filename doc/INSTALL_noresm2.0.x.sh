#!/bin/bash -e
noresm_version=2.0.9
if [ ! -f "release-noresm$noresm_version.tar.gz" ];then
    wget -c https://github.com/NorESMhub/NorESM/archive/refs/tags/release-noresm$noresm_version.tar.gz
fi
if [ ! -d "NorESM-release-noresm$noresm_version" ];then
    tar xzf release-noresm$noresm_version.tar.gz
fi
cd NorESM-release-noresm$noresm_version/
