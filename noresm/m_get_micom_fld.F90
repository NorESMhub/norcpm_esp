...
subroutine get_micom_fld_new(memfile,fld,cfld,vlevel,tlevel,nx,ny)
   ...
   if (ex) then
     ...
     print '(a)', 'ERROR: file is missing '//trim(memfile)//'.nc, var: '//trim(cfld)
     stop
  endif
end subroutine
...
subroutine get_micom_fld(memfile,fld,cfld,tlevel,nx,ny,nz)
   ...
   if (ex) then
     ...
     !print *, 'ERROR: forecast file is missing '//trim(memfile)//'.nc'
     print '(a)', 'ERROR: file is missing '//trim(memfile)//'.nc, fld: '//trim(cfld)
     stop
  endif
...
subroutine get_micom_fld_1d(memfile,fld,cfld,nz)
   ...
   if (ex) then
     ...
     print '(a)', 'ERROR: file is missing '//trim(memfile)//'.nc, fld1d: '//trim(cfld)
      stop
   endif
