...
contains
  ...
  subroutine nfw_open(fname, mode, ncid)
    ...
    if(trim(fname).eq.'TEM.nc' .or. trim(fname).eq.'SAL.nc')then
        write(logunit, "('blc, opening ',a,' in mode ',i4)")trim(fname),mode
    end if
    status = nf_open(trim(fname), mode, ncid)
    if (status /= 0) write(logunit, "('blc, opening ',a,' in mode ',i4)")trim(fname),mode
  ...
  subroutine nfw_def_var(fname, ncid, name, type, ndims, dimids, varid)
    ...
    !write(logunit, *)'blc, defining "',trim(name),'" in ',trim(fname), ndims, dimids(1:ndims)
    status = nf_inq_varid(ncid, trim(name), varid)
    if (status == 0) write(logunit, *)'blc, already defined: "',trim(name),'" in ',trim(fname)
