...
module m_prep_4_EnKF
  ...
  use norcpm_otf, only: gather_blom_1i1l,read_nc_file_2d_and_bcast_r8 &
                       ,save_2d_nc_fn_vn
    ...
    do iuobs = 1, nuobs
       ...
       if (trim(unique_obs(iuobs)) == 'ICEC') then
          do iens = 1, nrens
             ...
             !     'icec', 0, 1, nx, ny)
             call gather_blom_1i1l(iens,'icec',0,-1,readfld)
...
!Read the monthly mean 
          ...
          !! somehow aboves is in the iens loop, do not know why
          call read_nc_file_2d_and_bcast_r8('mean_mod.nc','sst',field2)
          !! check recived sst 
          !!call save_2d_nc_fn_vn('mean_modtemp_lev1','temp_lev1',field2,.true.)
#endif
          do iens = 1, nrens
             write(cmem,'(i3.3)') iens
             !call get_micom_fld_new(trim('forecast'//cmem), readfld, &
             !     'temp', 1, 1, nx, ny)
             call gather_blom_1i1l(iens,'temp',1,-1,readfld)
#ifdef ANOMALY
             readfld = readfld - field2
#endif
             !!call save_2d_nc_fn_vn('temp_lev1_'//cmem,'temp_lev1',readfld,.true.)
             call Generate_element_Si(S(:, iens), unique_obs(iuobs),&
                  readfld, depths, nx, ny, nz, 0) 
          end do
       ...
       elseif (trim(unique_obs(iuobs)) == 'SSH') then
          do iens = 1, nrens
             ...
             !call get_micom_fld_new(trim('forecast'//cmem), readfld, &
             !     'sealv', 1, 1, nx, ny)
             call gather_blom_1i1l(iens,'sealv',1,-1,readfld) !! 1 or 0 ?
             ! Convert to meter
             readfld = readfld/100.
             ...
             !call get_micom_fld_new('mean_mod', field2, &
             !     'sealv', 0, 1, nx, ny)
             call read_nc_file_2d_and_bcast_r8('mean_mod.nc','sealv',field2)
             readfld = readfld - field2
       ...
       elseif (trim(unique_obs(iuobs)) == 'SLA' .or.&
          ...
          do iens = 1, nrens
             ...
             !call get_micom_fld_new(trim('forecast'//cmem), readfld, &
             ! 'ssh', 1, 1, nx, ny)
             call gather_blom_1i1l(iens,'ssh',1,-1,readfld) !! 1 or 0 ?
             if (prm_prmestexists('msshb')) then
                write(cmem,'(i3.3)') iens
                !call get_micom_fld_new(trim('forecast'//cmem), field3,&
                !     'msshb', 0, 1, nx, ny)
                call gather_blom_1i1l(iens,'msshb',1,-1,field3) !! 1 or 0 ?
       ...
       elseif (trim(unique_obs(iuobs)) == 'SAL' .or.&
          ...
          if(master)print'(a,g0.3)','m_prep_4_EnKF.F90:237; timing, get_climato_spline() ',real(t1-t0)/real(cr)
          ...
          call system_clock(t1,cr)
          if(master)print'(a,g0.3)','m_prep_4_EnKF.F90:253; timing, get_S_spline() ',real(t1-t0)/real(cr)
          if (master) then
