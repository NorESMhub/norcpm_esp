...
module m_Generate_element_Si
  use norcpm_otf, only: gather_blom_1i1l, read_dim3_and_bcast &
            ,read_1d_and_bcast, read_nc_file_3d_and_bcast_r8
  implicit none
...
!    call parse_blkdat('kdm   ','integer', rdummy, nk)
    ...
    if (field == TEMPERATURE .and. prm_prmestexists('sstb')) then
       ...
       call gather_blom_1i1l(iens,'sstb',0,-1,sstbias)
       if (tlevel == -1) then
          if (master) then
       ...
       call gather_blom_1i1l(iens,'dpmixl',0,-1,mld)
       if (tlevel == -1) then
          if (master) then
    ...
    do k = 1, nk + 1
       ...
       if (k <= nk) then
          ...
          call gather_blom_1i1l(iens,'dp',0,-1,dz2d)
          if (tlevel == -1) then
             if (master) then
          ...
          call gather_blom_1i1l(iens,trim(fieldtag),0,-1,v2d)
          if (tlevel == -1) then
             if (master) then
  ...
  subroutine get_S_spline(S, obstag, nobs, obs, iens)
    ...
    call system_clock(t0,cr)
    do k = 1,nk
        call gather_blom_1i1l(iens,'dp',  k,-1,dz3d(:,:,k))
        call gather_blom_1i1l(iens,'temp',k,-1,temp(:,:,k))
        call gather_blom_1i1l(iens,'saln',k,-1,saln(:,:,k))
    end do
    call system_clock(t1,cr)
    if(master.and.timinglog)print'(a,g0.3)','m_Generate_element_Si.F90:590; timing, gather_blom_i11l() ',real(t1-t0)/real(cr)
...
!!$                 print *, ipiv(p), jpiv(p)
   ...
   subroutine get_climato_spline(S, obstag, nobs, obs)
    ...
    !! repalce with norcpm_otf
    call read_dim3_and_bcast('mean_mod.nc',ni,nj,nk)
    ...
    ! read the depth and the requested field at all layers
    !
    !! replace with norcpm_otf
    !call get_micom_fld_1d(trim(fname), dz3d, 'depth', nk)
    !call get_micom_fld(trim(fname), v3d, trim(fieldtag)//'lvl', tlevel, ni, nj, nk)
    call read_1d_and_bcast(trim(fname)//'.nc','depth',nk,dz3d)
    call read_nc_file_3d_and_bcast_r8(trim(fname)//'.nc',trim(fieldtag)//'lvl',v3d)
    ...
    do p = 1, nprof
       ...
       if (o1 .gt. o2+1) then
        ...
        !! SPLINE_PCHIP_SET - Fatal error!
        !!  Number of data points less than 2.
        !!  1
        !!if (o2 - o1 .ge. 0) then
        if (o2 - o1 .ge. 0) then
            if(m.lt.3)then
                print*,'get_climato_spline():900;o1,o2,m = ',o1,o2,m
            end if
           call spline_pchip_set(m, zcentre(1:m), v(1:m), vd(1:m))
           call spline_pchip_val(m, zcentre(1:m), v(1:m), vd(1:m), o2 - o1 + 1,&
                obs(o1:o2) % depth, S(o1:o2))
