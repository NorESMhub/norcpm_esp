...
! makes all processes sync at this point, optionally print a label
  ...
  subroutine qmpi_send_logical(data, target, tag)
    ...
    counter=1
    call mpi_send(data, counter, mpi_logical, target, given_tag, espcomm, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_send_string(data, target, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=len(data)
    call mpi_send(data, counter, mpi_character, target, given_tag, espcomm, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_recv_character_1d(data, target, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=size(data)
    call mpi_recv(data, counter, mpi_character, target, given_tag, espcomm, mpistatus, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_send_integer4(data, target, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=1
    call mpi_send(data, counter, mpi_integer, target, given_tag, espcomm, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_send_integer8(data, target, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=1
    call mpi_send(data, counter, mpi_integer8, target, given_tag, espcomm, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_send_real4(data, target, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=1
    call mpi_send(data, counter, mpi_real, target, given_tag, espcomm, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_send_real8(data, target, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=1
    call mpi_send(data, counter, mpi_double_precision, target, given_tag, espcomm, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_recv_integer4(data, source, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=1
    call mpi_recv(data, counter, mpi_integer, source, given_tag, espcomm, mpistatus, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_recv_integer8(data, source, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=1
    call mpi_recv(data, counter, mpi_integer8, source, given_tag, espcomm, mpistatus, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_recv_real4(data, source, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=1
    call mpi_recv(data, counter, mpi_real, source, given_tag, espcomm, mpistatus, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_recv_real8(data, source, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=1
    call mpi_recv(data, counter, mpi_double_precision, source, given_tag, espcomm, mpistatus, ierr)
    if(ierr.ne.0)then
  ...
  subroutine qmpi_recv_logical(data, target, tag)
    ...
    if(present(tag)) given_tag=tag
    counter=1
    call mpi_recv(data, counter, mpi_logical, target, given_tag, espcomm, mpistatus, ierr)
    if(ierr.ne.0)then
...
!
    ...
    call mpi_bcast(string , counter, mpi_character, boss, espcomm  ,ierr)
  ...
  subroutine qmpi_broadcast_stringarr(data,root)
    ...
    call mpi_bcast(data, counter, mpi_character, boss, espcomm ,ierr)
  ...
  subroutine qmpi_broadcast_real4(data,root)
    ...
    call mpi_bcast(data , counter, mpi_real, boss, espcomm, ierr)
  ...
  subroutine qmpi_broadcast_real8(data,root)
    ...
    boss=0
    if(present(root)) boss=root
    call mpi_bcast(data , counter, mpi_double_precision, boss, espcomm, ierr)
  ...
  subroutine qmpi_broadcast_integer4(data,root)
    ...
    boss=0
    if(present(root)) boss=root
    call mpi_bcast(data , counter, mpi_integer, boss, espcomm, ierr)
  ...
  subroutine qmpi_broadcast_integer8(data,root)
    ...
    boss=0
    if(present(root)) boss=root
    call mpi_bcast(data , counter, mpi_integer8, boss, espcomm, ierr)
  ...
  subroutine qmpi_broadcast_logical(data, root)
    ...
    boss=0
    if(present(root)) boss=root
    call mpi_bcast(data , counter, mpi_logical, boss, espcomm, ierr)
...
!    end if
    ...
    call mpi_bcast(rbuff , counter, mpi_real, boss, espcomm  ,ierr)
...
!
    ...
    call mpi_bcast(rbuff , counter, mpi_double_precision, boss, espcomm  ,ierr)
...
!
    ...
    call mpi_bcast(lbuff , counter, mpi_logical, boss, espcomm  ,ierr)
...
!    end if
    ...
    call mpi_bcast(ibuff , counter, mpi_integer, boss, espcomm  ,ierr)
...
!    end if
    ...
    call mpi_bcast(ibuff , counter, mpi_integer8, boss, espcomm  ,ierr)
...
!    end if
    ...
    case('sum')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter,mpi_double_precision,mpi_sum,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter,mpi_double_precision,mpi_sum,espcomm,ierr)
        end if
    case('mul')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter,mpi_double_precision,mpi_prod,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter,mpi_double_precision,mpi_prod,espcomm,ierr)
        end if
    case('min')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter,mpi_double_precision,mpi_min,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter,mpi_double_precision,mpi_min,espcomm,ierr)
        end if
    case('max')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter,mpi_double_precision,mpi_max,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter,mpi_double_precision,mpi_max,espcomm,ierr)
        end if
    end select
...
!    
    ...
    case('sum')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter, MPI_INTEGER, mpi_sum,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter, MPI_INTEGER, mpi_sum,espcomm,ierr)
        end if
    case('mul')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter, MPI_INTEGER, mpi_prod,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter, MPI_INTEGER, mpi_prod,espcomm,ierr)
        end if
    case('min')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter, MPI_INTEGER, mpi_min,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter, MPI_INTEGER, mpi_min,espcomm,ierr)
        end if
    case('max')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter, MPI_INTEGER, mpi_max,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter, MPI_INTEGER, mpi_max,espcomm,ierr)
        end if
    end select
...
!    end if
    ...
    case('sum')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter, MPI_INTEGER8, mpi_sum,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter, MPI_INTEGER8, mpi_sum,espcomm,ierr)
        end if
    case('mul')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter, MPI_INTEGER8, mpi_prod,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter, MPI_INTEGER8, mpi_prod,espcomm,ierr)
        end if
    case('min')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter, MPI_INTEGER8, mpi_min,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter, MPI_INTEGER8, mpi_min,espcomm,ierr)
        end if
    case('max')
        if(present(root))then
            call mpi_reduce(rbuff,globrbuff,counter, MPI_INTEGER8, mpi_max,boss,espcomm,ierr)
        else
            call mpi_allreduce(rbuff,globrbuff,counter, MPI_INTEGER8, mpi_max,espcomm,ierr)
        end if
    end select
