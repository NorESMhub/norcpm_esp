...
subroutine pivotp_micom_new(lon, lat,modlon,modlat, ipiv, jpiv, nx, ny, min_r, max_r, &
  ...
  if (jpiv == ny) then
     ...
     if (inside) then
        ...
        print *, 'model lonlat(ipiv  , jpiv)   = ', modlon(ipiv,jpiv), modlat(ipiv,jpiv)
        print *, 'model lonlat(ipiv+1, jpiv)   = ', modlon(ipiv+1,jpiv), modlat(ipiv+1,jpiv)
        print *, 'model lonlat(ipiv  , jpiv+1) = ', modlon(ipiv,jpiv+1), modlat(ipiv,jpiv+1)
        print *, 'model lonlat(ipiv+1, jpiv+1) = ', modlon(ipiv+1,jpiv+1), modlat(ipiv+1,jpiv+1)
        print *, 'obs lonlat: ',lon, lat
        stop
     end if
  else
     return
...
subroutine check_point(x, y, xx, yy, inside)
  ...
  if (debug)then
      write(*,'(a,4f8.3)')'check_point(): xx = ',xx(1),xx(2),xx(3),xx(4)
      write(*,'(a,5f8.3)')'               yy = ',yy(1),yy(2),yy(3),yy(4)
      write(*,'(a,2f8.3)')'              x,y = ',x,y
  end if
  ...
  sum_triangle_area = sum_triangle_area + Herons_formula(a,b,c)
  if(debug)then
      s = (a + b + c) / 2
      sum_triangle_area = sum_triangle_area + max(0.,sqrt(s*(s-a)*(s-b)*(s-c)))
      write(*,'(a,5f8.3)')'    s,a,b,c,h = ',s,a,b,c,(s*(s-a)*(s-b)*(s-c))
      write(*,'(a,f8.3)') '    tri area = ',sqrt(s*(s-a)*(s-b)*(s-c))
  end if
 ...
 ! fourth trangle area with (xx(1), yy(1)), (xx(4), yy(4)) and (x,y)
  ...
  if(debug)then
      ...
      sum_triangle_area = sum_triangle_area + max(0.,sqrt(s*(s-a)*(s-b)*(s-c)))
      write(*,'(a,5f8.3)')'    s,a,b,c,h = ',s,a,b,c,(s*(s-a)*(s-b)*(s-c))
      write(*,'(a,f8.3)') '    tri area = ',max(0.,sqrt(s*(s-a)*(s-b)*(s-c)))
      write(*,*) '    s,c,s-c = ',s,c,s-c
  end if
  ...
  if(debug)then
      write(*,'(a,f8.3,a,f8.3)'),'    sum_tri,rect areas = ' &
          ,rectangle_area,',',sum_triangle_area
  end if
...
function Herons_formula(a,b,c)
    !! cal triangle area with 3 edge
    real :: Herons_formula
    real, intent(in):: a,b,c
    real :: s
    s = (a + b + c) / 2.
    Herons_formula = sqrt(s*(s-a)*(s-b)*(s-c))
    if (isnan(Herons_formula))Herons_formula = 0.
    return
end function Herons_formula
